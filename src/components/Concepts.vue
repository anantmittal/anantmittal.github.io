...
<script>
	...
	export default {
		...
		mounted: function() {
			this.emitPageLoaded(this.$route.params.id);
		},
		methods: {
			fetchData() {
				axios
					.get('data/' + this.$route.params.id + '/Concepts.json')
					.then((resp) => {
				...
				axios
					.get('data/' + this.$route.params.id + '/MathProblem.json')
					.then((resp) => {
