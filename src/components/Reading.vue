<template>
	<div>
		<b-container class="bv-example-row" style="text-align:justify;">
			<b-row>
				<b-col>
					<h3>Reading</h3>
					<ul>
						<li>
							Jugalbandi: The BJP Before Modi - Vinay Sitapati
						</li>
						<li>
							The Feynman Lectures on Physics - Richard Feynman,
							Robert B. Leighton, Matthew Sands
						</li>
						<li>
							The Essential Dykes to Watch Out For - Alison
							Bechdel
						</li>
						<li>A Promised Land - Barack Obama</li>
					</ul>
					<h3>2021</h3>
					<ul>
						<li>Are You My Mother? - Alison Bechdel</li>
						<li>
							Lost in Thought: The Hidden Pleasures of an
							Intellectual Life - Zena Hitz
						</li>
						<li>My President Was Black - Ta-Nehisi Coates</li>
						<li>The Warmth of Other Suns - Isabel Wilkerson</li>
						<li>Caste - Isabel Wilkerson</li>
						<li>Ghachar Ghochar - Vivek Shanbhag</li>
						<li>Caste Matters - Suraj Yengde</li>
						<li>Joothan - Om Prakash Valmiki</li>
						<li>Coming Out as Dalit - Yashica Dutt</li>
						<li>Fun Home - Alison Bechdel</li>
						<li>
							Mindset: The New Psychology of Success - Carol Dweck
						</li>
						<li>
							The Untouchables : Who were they and why they Became
							Untouchables - B R Ambedkar
						</li>
					</ul>
					<h3>2020</h3>
					<ul>
						<li>
							21 Lessons for the 21st Century - Yuval Noah Harari
						</li>
						<li>
							Thinking Fast and Slow - Daniel Kahneman
						</li>
						<li>Midnight in Chernobyl - Adam Higginbotham</li>
						<li>Mindstorms - Seymour Papert</li>
						<li>The Snow Child - Eowyn Ivey</li>
						<li>
							No Visible Bruises: What We Don't Know About
							Domestic Violence Can Kill Us - Rachel Louise Snyder
						</li>
						<li>
							Thick: And Other Essays - Tressie McMillan Cottom
						</li>
						<li>
							Nudge: Improving Decisions About Health, Wealth, and
							Happiness - Richard H. Thaler
						</li>
						<li>The Body: A Guide for Occupants - Bill Bryson</li>
						<li>
							Between the World and Me - Ta-Nehisi Coates
						</li>
						<li>
							The Handmaid's Tale: The Graphic Novel - Margaret
							Atwood
						</li>
						<li>
							Where the Crawdads Sing Novel - Delia Owens
						</li>
						<li>
							Geek Heresy: Rescuing Social Change from the Cult of
							Technology - Kentaro Toyama
						</li>
						<li>
							The Privileged Poor: How Elite Colleges Are Failing
							Disadvantaged Students - Anthony Abraham Jack
						</li>
						<li>
							Rebooting AI: Building Artificial Intelligence We
							Can Trust - Gary Marcus
						</li>
						<li>Quichotte - Salman Rushdie</li>
						<li>Night Boat to Tangier - Kevin Barry</li>
						<li>
							The Decameron Project: 29 New Stories from the
							Pandemic
						</li>
						<li>Black Panther Series - Ta-Nehisi Coates</li>
						<li>
							White Fragility: Why It's So Hard for White People
							to Talk About Racism - Robin DiAngelo
						</li>
					</ul>
				</b-col>
			</b-row>
		</b-container>
	</div>
</template>

<script>
	export default {
		name: "Reading",
		props: {
			msg: String,
		},
		data() {
			return {};
		},
		components: {},
		mounted: function() {},
		created() {},
		methods: {},
	};
</script>
